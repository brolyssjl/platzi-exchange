<template>
  <div class="flex-col">
    ...
    <template v-if="!isLoading">
          ...
          <div class="flex flex-row my-5">
            <label class="w-full" for="convertValue">
              <input
                ...
                :placeholder="fromUsd ? `Valor en ${ asset.symbol }` : 'Valor en USD'"
              />
            </label>
          </div>

          <span class="text-xl">{{ convert }} {{ fromUsd ? asset.symbol : 'USD' }}</span>
        </div>
      </div>
      ...
      <img alt="joder" class="red" :class="{ blue: true, green: false, yellow: 1 > 0 }" />
    </template>
  </div>
</template>
...
export default {
  ...
  methods: {
    getWebsite (exchange) {
      ...
      .then(res => {
        //esto ayuda a solucionar problemas de reactividad
        //cuando un dato no se declara en la creación del componente, es decir,
        // que ya existe
        // como es el caso de arrays y objetos
        // y con ayuda $set podemos solucionar el problema
        // Tres parámetros son los que necesita
        // 1. El objeto
        // 2. El nombre del propiedad
        // 3. El valor de esa propiedad
